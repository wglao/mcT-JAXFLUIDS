...
def pos_coefs(seed: int, out_shape: Union[tuple, int]) -> jnp.ndarray:
    ...
    if type(out_shape) == int:
        # strictly positive
        return jnp.abs(jnp.sqrt(0.5)*jrand.truncated_normal(key, -jnp.sqrt(0.5), jnp.sqrt(0.5), (out_shape,)) + 0.5) + jnp.finfo(jnp.float32).eps
    # strictly positive
    return jnp.abs(jnp.sqrt(0.5)*jrand.truncated_normal(key, -jnp.sqrt(0.5), jnp.sqrt(0.5), out_shape) + 0.5) + jnp.finfo(jnp.float32).eps
...
if __name__ == "__main__":
    ...
    def warm_loss(params, cons_L, cons_R, truth):
        ...
        for i in range(5):
            net_L = jax.jit(jax.vmap(net.apply, in_axes=(None, 0)))(
                params[i], cons_L[:, i])
            net_R = jax.jit(jax.vmap(net.apply, in_axes=(None, 0)))(
                params[i], cons_R[:, i])
            tangent_i = jnp.reshape(
                0.5*(net_L+net_R), net_tangent[:, i, ...].shape)
    ...
    def warm_load(sim: dat.Sim, sim_manager: SimulationManager, epoch: int):
        ...
        cons_L = jnp.array(jax.vmap(get_conservatives_from_primitives, in_axes=(
            0, None))(primes_L, sim_manager.material_manager))
        cons_R = jnp.array(jax.vmap(get_conservatives_from_primitives, in_axes=(
            0, None))(primes_R, sim_manager.material_manager))
    ...
    def warm_start(epochs):
        ...
        for epoch in range(epochs):
            ...
            # load data
            primes_L, primes_R, cons_L, cons_R = warm_load(
                sim, sim_manager, epoch)
            ...
            warm_true = jax.vmap(
                model.solve_riemann_problem_xi, in_axes=(0, 0, 0, 0, None))
            truth_array = jnp.array(
                warm_true(primes_L, primes_R, cons_L, cons_R, 0))
            loss, grads = jax.value_and_grad(jit(warm_loss), argnums=(0))(
                params, cons_L, cons_R, truth_array)
            for i in range(5):
                updates_i, opt_state[i] = jit(
                    optimizer.update)(grads[i], opt_state[i])
            ...
            if epoch % 500 == 0 or epoch == epochs-1:
                print("Loss {:.2e} TE {:.2e}  TE_min {:.2e} EPmin {:d} EP {} ".format(
                    loss, test_err, min_err, epoch_min, epoch))

            if epoch % 200 == 0 and epoch > 0:
                jax.clear_backends()
            wandb.log({"Train loss": float(loss), "Test Error": float(
                test_err), 'Test Min': float(min_err), 'Epoch': float(epoch)})
            # clean up
            os.system('rm -rf %s' % (cache_path))

        save_params(params, os.path.join(
            proj("network/parameters"), "warm.pkl"))
