...
class Data():
    ...
    def next_sim(self) -> Sim:
        ...
        sim_name = re.split(r'-\d+', sim_path)[0] if re.split(
            r'-\d+', sim_path) else sim_path
        domain = os.path.join(self.save_path, sim_path, 'domain')

        f = open(os.path.join(self.save_path, sim_path, sim_name + '.json'),
                 'r')
        case_setup = json.load(f)
        f.close()

        f = open(
            os.path.join(self.save_path, sim_path, 'numerical_setup.json'),
            'r')
        ...
        if self.size() < setup.cases.size():
            ...
            for _ in range(n_to_gen):
                ...
                if self.size() >= setup.num_train:
                    case_setup['general'][
                        'end_time'] = setup.t_max * setup.test_ratio
    ...
    def calc_analytical(self, fx0, xs, t):
        gamma = 1.4
        ...
        shock_speed = a1 * jnp.sqrt((gamma + 1) / 2 / gamma *
                                    (p2 / p1 - 1) + 1)
        ...
        def expansion_wave(x, t):
            u = jnp.where(
                t > 0, 2 / (1 - gamma) * (x / jnp.where(t > 0, t, 0.1) + a4),
                u4)
            return adiabatic_expansion(u, a4)

        post_shock = jnp.array([
            rho1 * (1 + (gamma + 1) / (gamma - 1) * p2 / p1) /
            ((gamma + 1) / (gamma - 1) + p2 / p1), up, p2
        ])
            ...
            x_prime = x_prime.at[:].set(
                jnp.where(
                    ...
                    jnp.where(
                        ...
                        jnp.where(
                            x <= contact_x, adiabatic_expansion(up, a4),
    ...
    def gen_analytical(self, reset=False):
        ...
        if self.size() < setup.cases.size():
            ...
            for _ in range(n_to_gen):
                ...
                fx0 = jnp.zeros((5, 2))
                for i, var0 in enumerate(
                        case_setup['initial_condition'].values()):
                    if type(var0) == str:
    ...
    def load_all(self):
        ...
        data_load = np.zeros((setup.num_test, 4, int(setup.nt * 2) + 1,
