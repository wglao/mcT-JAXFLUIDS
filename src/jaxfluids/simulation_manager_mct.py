...
class SimulationManagerMCT:
    ...
    def __init__(self, input_reader: InputReader) -> None:
        ...
        config.update(
            "jax_enable_x64", self.numerical_setup["output"]["is_double_precision_compute"])
        ...
        self.unit_handler = UnitHandler(
            **input_reader.nondimensionalization_parameters)
        ...
        self.domain_information = DomainInformation(
            ...
            domain_size=self.unit_handler.non_dimensionalize_domain_size(
                input_reader.domain_size)
        )
        ...
        if "fixed_timestep" in self.numerical_setup["conservatives"]["time_integration"].keys():
            self.fixed_timestep = self.unit_handler.non_dimensionalize(
                self.numerical_setup["conservatives"]["time_integration"]["fixed_timestep"], "time")
        else:
            self.fixed_timestep = False
        self.end_time = self.unit_handler.non_dimensionalize(
            input_reader.end_time, "time")
        self.CFL = self.numerical_setup["conservatives"]["time_integration"]["CFL"]

        self.time_integrator: TimeIntegrator = DICT_TIME_INTEGRATION[self.numerical_setup["conservatives"]["time_integration"]["integrator"]](
            nh=self.domain_information.nh_conservatives, inactive_axis=self.domain_information.inactive_axis)
        ...
        self.space_solver = SpaceSolver(
            ...
            material_manager=self.material_manager,
            numerical_setup=self.numerical_setup,
            gravity=self.unit_handler.non_dimensionalize(
                input_reader.gravity, "gravity"),
        ...
        self.output_writer = OutputWriter(
            ...
            material_manager=self.material_manager,
            levelset_handler=self.levelset_handler if self.input_reader.levelset_type != None else None,
            derivative_stencil_conservatives=DICT_FIRST_DERIVATIVE_CENTER[self.numerical_setup["output"]["derivative_stencil"]](
                nh=self.domain_information.nh_conservatives, inactive_axis=self.domain_information.inactive_axis),
            derivative_stencil_geometry=DICT_FIRST_DERIVATIVE_CENTER[self.numerical_setup["output"]["derivative_stencil"]](
                nh=self.domain_information.nh_geometry, inactive_axis=self.domain_information.inactive_axis) if self.input_reader.levelset_type != None else None,
        )

        self.logger = Logger(
            "", logging_level=self.numerical_setup["output"]["logging"])
    ...
    def advance(self, buffer_dictionary: Dict[str, Dict[str, Union[jnp.DeviceArray, float]]]) -> None:
        ...
        while current_time < self.end_time - self.eps_time:
            ...
            self.output_writer.write_output(
                buffer_dictionary, force_output=False)
            ...
            mean_wall_clock_step = (wall_clock_step + mean_wall_clock_step*(
                current_step - 1))/current_step if current_step > 3 else wall_clock_step
            mean_wall_clock_step_cell = (wall_clock_step_cell + mean_wall_clock_step_cell*(
                current_step - 1))/current_step if current_step > 3 else wall_clock_step_cell
            ...
            print_list = [
                'CURRENT TIME                   = %4.4e' % (
                    self.unit_handler.dimensionalize(current_time, "time")),
                'CURRENT DT                     = %4.4e' % (
                    self.unit_handler.dimensionalize(timestep_size, "time")),
                'CURRENT STEP                   = %6d' % current_step,
                'WALL CLOCK TIMESTEP            = %4.4e' % wall_clock_step,
                'MEAN WALL CLOCK TIMESTEP       = %4.4e' % mean_wall_clock_step,
                'WALL CLOCK TIMESTEP CELL       = %4.4e' % (
                    wall_clock_step_cell),
                'MEAN WALL CLOCK TIMESTEP CELL  = %4.4e' % (
                    mean_wall_clock_step_cell)
            ]

            if self.input_reader.levelset_type != None:
                print_list += ['RESIDUAL EXTENSION PRIMES      = %4.4e' %
                               residuals["extension_primes"]]
            if self.input_reader.levelset_type == "FLUID-FLUID":
                print_list += ['RESIDUAL EXTENSION INTERFACE   = %4.4e' %
                               residuals["extension_interface"]]
                print_list += ['RESIDUAL REINITIALIZATION      = %4.4e' %
                               residuals["reinitialization"]]

            self.logger.log_end_time_step(print_list)

        # FINAL OUTPUT
        self.output_writer.write_output(
            buffer_dictionary, force_output=True, simulation_finish=True)
    ...
    def do_integration_step(self, cons: jnp.DeviceArray, primes: jnp.DeviceArray, timestep_size: float, current_time: float,
        ...
        residuals = {"extension_primes": residual_primes,
                     "extension_interface": residual_interface, "reinitialization": residual_reinit}
    ...
    def compute_timestep(self, primes: jnp.DeviceArray, levelset: jnp.DeviceArray,
        ...
        min_cell_size = jnp.min(
            jnp.array([jnp.min(dxi) for dxi in cell_sizes]))
        ...
        if self.numerical_setup["active_physics"]["is_viscous_flux"] or self.numerical_setup["active_physics"]["is_heat_flux"]:
            temperature = self.material_manager.get_temperature(
                primes[4, ..., nhx, nhy, nhz], primes[0, ..., nhx, nhy, nhz])

        # COMPUTE MASKS
        if self.input_reader.levelset_type != None:
            mask_real, _ = self.levelset_handler.compute_masks(
                levelset, volume_fraction)

        # CONVECTIVE CONTRIBUTION
        speed_of_sound = self.material_manager.get_speed_of_sound(
            p=primes[4, ..., nhx, nhy, nhz], rho=primes[0, ..., nhx, nhy, nhz])
        abs_velocity = 0.0
        for i in range(1, 4):
            abs_velocity += (jnp.abs(primes[i, ...,
                             nhx, nhy, nhz]) + speed_of_sound)
        if self.input_reader.levelset_type != None:
        ...
        if self.numerical_setup["active_physics"]["is_viscous_flux"]:
            ...
            kinematic_viscosity = self.material_manager.get_dynamic_viscosity(
                temperature) / primes[0, ..., nhx, nhy, nhz]
            if self.input_reader.levelset_type != None:
                kinematic_viscosity = kinematic_viscosity * \
                    mask_real[..., nhx_, nhy_, nhz_]
            dt = jnp.minimum(dt, const * (min_cell_size *
                             min_cell_size) / jnp.max(kinematic_viscosity))
        ...
        if self.numerical_setup["active_physics"]["is_heat_flux"]:
            ...
            if self.input_reader.levelset_type != None:
                thermal_diffusivity = thermal_diffusivity * \
                    mask_real[..., nhx_, nhy_, nhz_]
            dt = jnp.minimum(dt, const * (min_cell_size *
                             min_cell_size) / jnp.max(thermal_diffusivity))
    ...
    def _feed_forward(self, primes_init: jnp.DeviceArray, levelset_init: jnp.DeviceArray,
        ...
        # CREATE BUFFER
        if self.input_reader.levelset_type == "FLUID-FLUID":
            primes = jnp.ones((5, 2, nx + 2*nh if nx > 1 else nx, ny +
                              2*nh if ny > 1 else ny, nz + 2*nh if nz > 1 else nz))
            cons = jnp.zeros((5, 2, nx + 2*nh if nx > 1 else nx, ny +
                             2*nh if ny > 1 else ny, nz + 2*nh if nz > 1 else nz))
            levelset = jnp.zeros((nx + 2*nh if nx > 1 else nx, ny +
                                 2*nh if ny > 1 else ny, nz + 2*nh if nz > 1 else nz))
        else:
            primes = jnp.ones((5, nx + 2*nh if nx > 1 else nx, ny +
                              2*nh if ny > 1 else ny, nz + 2*nh if nz > 1 else nz))
            cons = jnp.zeros((5, nx + 2*nh if nx > 1 else nx, ny +
                             2*nh if ny > 1 else ny, nz + 2*nh if nz > 1 else nz))
            levelset = None
        ...
        if self.input_reader.levelset_type == "FLUID-FLUID":
            ...
            levelset = self.boundary_condition.fill_boundary_levelset(levelset)
            volume_fraction, apertures = self.levelset_handler.compute_volume_fraction_and_apertures(
                levelset)
            _, primes, _ = self.levelset_handler.extend_primes(
                cons, primes, levelset, volume_fraction, t_start)
        else:
        ...
        if self.input_reader.levelset_type != None:
            primes_real = self.output_writer.compute_real_buffer(
                primes[..., nhx, nhy, nhz], volume_fraction[nhx_, nhy_, nhz_])
            out = jnp.concatenate([primes_real, jnp.expand_dims(
                volume_fraction[nhx_, nhy_, nhz_], axis=0)], axis=0)
        else:
            primes_real = primes[:, nhx, nhy, nhz]
            out = primes_real
        ...
        for step in jnp.arange(n_steps):
            ...
            primes, cons = material_fields["primes"], material_fields["cons"]
            levelset, volume_fraction, apertures = levelset_quantities[
                "levelset"], levelset_quantities["volume_fraction"], levelset_quantities["apertures"]
            ...
            if current_step % output_freq == 0:
                if self.input_reader.levelset_type != None:
                    cons_real = self.output_writer.compute_real_buffer(
                        cons[..., nhx, nhy, nhz], volume_fraction[nhx_, nhy_, nhz_])
                    primes_real = self.output_writer.compute_real_buffer(
                        primes[..., nhx, nhy, nhz], volume_fraction[nhx_, nhy_, nhz_])
                    out = jnp.concatenate([primes_real, jnp.expand_dims(
                        volume_fraction[nhx_, nhy_, nhz_], axis=0)], axis=0)
